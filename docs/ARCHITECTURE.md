...
reconciliation/  периодическая сверка локального состояния с биржей, обнаружение ADL. Сегодня в модуле
                 реально существует только recoverOnStartup.ts (findUnresolvedIntents + чистая
                 decideRecoveryOutcome) — логика перехода IDLE→RECOVERY→{IDLE|OPEN|STUCK_LEG}, полностью
                 протестированная, но нигде не вызываемая: реального аутентифицированного клиента биржи
                 для запроса ExchangeSnapshot ещё не существует (см. таблицу переходов §4 и её сноску)
...
**Статус реализации (2026-08-07):** три исхода `RECOVERY` выше (`CONFIRMED_FLAT`→`IDLE`, `CONFIRMED_BOTH_LEGS`→`OPEN`, `CONFIRMED_ONE_LEG`→`STUCK_LEG`) реализованы как чистая функция `decideRecoveryOutcome` в `src/reconciliation/recoverOnStartup.ts`, вместе с `findUnresolvedIntents` — реальным (не заглушкой) запросом к уже смигрированным `positions`/`position_intents` за последним незавершённым намерением (`closed_at IS NULL`, без фильтра по `opened_at`, чтобы не пропустить крах ровно в `LEG1_SENT`, до того как позиция вообще "открылась"). Что НЕ реализовано и намеренно оставлено интерфейсом-заглушкой (`ExchangeSnapshot`): сам запрос к бирже за фактическим состоянием ног — аутентифицированного `PrivateExchangeClient` в кодовой базе пока не существует (нет и самого скрипта отправки ордеров), поэтому подключать `decideRecoveryOutcome` к реальному циклу `IDLE→RECOVERY` преждевременно. Модуль нигде не вызывается ни `trader`, ни `killswitch-listener` — это подготовленный, протестированный, но неактивный код, а не работающая функция восстановления.

**Статус реализации, 5 инвариантов выше (2026-08-07):** все пять реализованы как чистые функции в `src/reconciliation/checkInvariants.ts` (`checkPositionCountMatch`, `checkResidualDelta`, `checkNoUnexplainedShrinkage`, `checkNoStuckOrders`, `checkLivePositionLeverage`/`checkLiveAccountMMRate`), плюс `evaluateReconciliationCycle` — сводит найденные нарушения к одному из `RECONCILED_OK`/`RECONCILIATION_MISMATCH`/`FREEZE_TRIGGER` (нарушения по инварианту 5 приоритетнее нарушений по 1/2/3 — собственное решение этого кода, ARCHITECTURE.md явно не определяет порядок при одновременном срабатывании). Инвариант 5 намеренно НЕ переизобретает пороги 1.5x/0.30 — вызывает `risk/leverage.ts`'s `checkLeverage`/`checkAccountMMRate` напрямую, чтобы порог жил в одном месте. `findOpenPositions`/`findPendingOrders` — реальные (не заглушки) запросы к уже смигрированным `positions`/`orders`. То же самое разделение и тот же статус, что и `recoverOnStartup.ts` выше: «что на бирже сейчас» — инъецируемый типизированный параметр (`ExchangePerpHolding`), не живой вызов; модуль нигде не вызывается ни `trader`, ни `killswitch-listener`; ни одного импорта `bybit-api` (проверено отдельным адверсариальным ревью на соответствие NFR-11).
