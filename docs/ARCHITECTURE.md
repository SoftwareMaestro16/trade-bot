...
reconciliation/  периодическая сверка локального состояния с биржей, обнаружение ADL. Сегодня в модуле
                 реально существуют recoverOnStartup.ts (findUnresolvedIntents + чистая
                 decideRecoveryOutcome — логика перехода IDLE→RECOVERY→{IDLE|OPEN|STUCK_LEG}) и
                 checkInvariants.ts (пять инвариантов периодической сверки из §4 ниже, плюс
                 evaluateReconciliationCycle) — оба полностью протестированы, но нигде не вызываются:
