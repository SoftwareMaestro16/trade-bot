...
| `killswitch-listener` | `src/killswitch-listener.ts` | **Работает** — отдельный OS-процесс (RR-31), два независимых пути уровня 1 (файл-флаг + Telegram `/stop`, `/flatten`, `/resume`, `/status`). Живой прогон подтверждён: self-test, взведение по флагу, запись в `halt_state`, снятие только явной командой |
...
**Статус: VPS ещё не выбран и не арендован.** Раздел описывает процедуру, которая понадобится, когда он появится — решение сознательно отложено до момента, когда сборщик готов реально стартовать (см. переписку по Фазе 0/1: VPS не нужен для написания кода, нужен только для запуска настоящих двух недель непрерывного сбора).
...
```bash
git clone <repo> /opt/trade-bot   # приватный remote, когда он появится — сейчас репозиторий только локальный
cd /opt/trade-bot
npm ci
npm run build
...
**Код готов, аккаунт — нет.** `src/collector.ts` умеет пинговать внешний heartbeat (`notify/healthcheck.ts`), но это не заменяет SRS RR-35 (уровень 3 kill switch, требующий отдельный хост, RISK-REGISTER.md AR-09) — это просто дешёвый dead-man's-switch, независимый канал от Telegram-дайджеста (если сам процесс/бот недоступен настолько, что не может прислать дайджест, он не может прислать и ЭТО — вот именно этот случай ловит внешний сервис). Осталось:

1. Завести бесплатный проект на healthchecks.io (или аналоге), получить ping URL.
2. Вписать его в `.env` как `HEALTHCHECK_PING_URL` (см. `.env.example`).
3. Перезапустить `trade-bot-collector` — при следующем старте лог покажет `[startup] external heartbeat enabled, every 5min` вместо `disabled`.

Без `HEALTHCHECK_PING_URL` heartbeat просто не запускается (не ошибка, тихий no-op) — до этого шага **единственный способ узнать, что сборщик упал, — зайти и проверить руками**:
...
Провал этой проверки в CI (когда CI появится — сейчас не настроен, OPEN-05 в SRS §7) должен блокировать мердж, не быть предупреждением.
...
- **Kill switch — уровни 2 и 3, раздел 7 брифа.** Уровень 1 (файл-флаг + Telegram `/stop`/`/flatten`/`/resume`/`/status`, отдельный процесс `src/killswitch-listener.ts`) уже написан, протестирован и описан в разделе 2 выше. Уровень 2 (автоматические триггеры — просадка, `accountMMRate`, потеря WS-соединения) не написан. Уровень 3 (внешний watchdog-хост, RR-35) не написан. И ни один уровень пока не может физически ничего закрыть на бирже — `execution/` не размещает ордеров.
...
- **CI-пайплайн.** OPEN-05 в SRS §7 — открытый параметр, ответ за владельцем.
