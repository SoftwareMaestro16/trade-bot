 ...
 * gating the ping on. `dbStaleAfterMs`/`slowStreamStaleAfterMs` are collector.ts's
 * own DB_STALE_AFTER_MS/SLOW_STREAM_STALE_AFTER_MS constants, passed in rather
 * than imported so this stays decoupled from collector.ts the same way
 * notify/healthcheck.ts's own FreshnessCheck.latestAt is decoupled from any
 * particular table shape.
 */
export function buildFreshnessChecks(dbStaleAfterMs: number, slowStreamStaleAfterMs: number): FreshnessCheck[] {
  return [
    {
      label: "tickers",
