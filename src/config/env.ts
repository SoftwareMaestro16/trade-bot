...
const envSchema = z.object({
  ...
  // OpenRouter (или совместимый) ключ для LLM-АННОТАЦИЙ — резюме отчётов и
  // описание состояния рынка. Строго опционален и строго вне торгового
  // контура: если ключа нет, отчёт всё равно уходит, просто без текстового
  // резюме (см. analysis/llm/). LLM НИКОГДА не участвует в решении о входе —
  // risk/ остаётся детерминированным и юнит-тестируемым (ARCHITECTURE.md §2).
  LLM_API_KEY: z.string().optional(),
  // Модель для резюме. Дефолт задаётся в analysis/llm/client.ts, не здесь — эта схема
  // не решает, какая модель "правильная", только принимает переопределение.
